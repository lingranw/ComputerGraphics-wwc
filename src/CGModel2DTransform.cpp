...
#include "CG2022114533汪维诚Doc.h" // 包含View之前要包含Doc
#include "CG2022114533汪维诚View.h" // 用户交互绘制，并将图形对象通过Doc添加到场景
#include <iostream>
...
int CGModel2DTransform::OnMouseButton(GLFWwindow* window, int button, int action, int mods) {
	...
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
		...
		if (mods & GLFW_MOD_SHIFT) {
			...
			CCG2022114533汪维诚View* view = (CCG2022114533汪维诚View*)glfwGetWindowUserPointer(window);
			if (view) {
				// 转换屏幕坐标 (DCS) → 世界坐标 (WCS)
