...
// 创建正方体
void CCG2022114533汪维诚Doc::OnCubeCreate()
{
	//长方体（模型） 
...
// 创建球体
void CCG2022114533汪维诚Doc::OnSphereCreate()
{
	SphereParamsDialog dlg;
	dlg.mRadius = 50.0f;   // 默认值
	dlg.mSlices = 32;
	dlg.mStacks = 16;

	if (dlg.DoModal() == IDOK) {
		// 创建球体模型
		auto sphere = std::make_shared<CGSphere>(dlg.mRadius);
		auto hints = std::make_shared<TessellationHints>();
		hints->setTargetSlices(dlg.mSlices);
		hints->setTargetStacks(dlg.mStacks);
		...
		color1->setValue(glm::vec4(0.85f, 0.64f, 0.12f, 1.0f)); 
		...
		color2->setValue(glm::vec4(0.96f, 0.44f, 0.23f, 1.0f));
